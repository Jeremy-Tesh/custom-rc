...
export function sendSMS(data) {
	check(data.phoneNo, String);
	check(data.content, String);
	const options = {
		method: 'GET',
		hostname: 'sms.dial4sms.com',
		port: 6005,
		path: `/api/v2/SendSMS?ApiKey=mPggm7oI27AigJ2DzNRLnSAYqagT3YRpL7z%20kZax89o=&TemplateId=1207164499001767703&PrincipleEntityId=1301158193985470641&MobileNumbers=${
			data.phoneNo
		}&Message=Your%20OTP%20is%20${data.content.slice(
			36,
		)}%20-%20RDCILY&Is_Flash=false&Is_Unicode=false&SenderId=RDCILY&ClientId=993325ea-b686-4430-8f7d-d5263b3fc0ee`,
		headers: {
...
export async function sendv1PDFEmail(buffers, param) {
	...
	Email.send({
		from: 'jermitesh_ext@mongrov.com',
		to: 'jermitesh20@gmail.com',
		cc: param.cc,
...
API.v1.addRoute(
	...
	{ authRequired: false },
	{
		post() {
			...
			if (type?.toLowerCase().indexOf('sms') !== -1) {
				sendSMS({
					phoneNo: to,
					content: body,
				});
...
API.v1.addRoute(
	...
	{ authRequired: false },
	{
		post() {
			...
			console.log('FSR sent for ', param.call_no);
			doc.end();
		},
	},
);
