...
Meteor.startup(async function () {
	...
	if (process.env.TEST_MODE === 'true') {
		...
		if (Users.findOneByEmailAddress(adminUser.emails[0].address)) {
			throw new Meteor.Error(`Email ${adminUser.emails[0].address} already exists`, "Collaboration can't run in test mode");
		}

		if (!checkUsernameAvailability(adminUser.username)) {
			throw new Meteor.Error(`Username ${adminUser.username} already exists`, "Collaboration can't run in test mode");
		}
